...
(define-fun
  mkQ2
    ...
    (match x
      (case Nil2
        (match (halve2 y) (case (Pair22 bs cs) (Q22 (reverse2 bs) cs))))
      (case (Cons2 z x2)
        (match y
          (case Nil2
            (match (halve2 x)
...
(define-fun
  mkQ
    ...
    (match x
      (case Nil
        (match (halve y) (case (Pair2 bs cs) (Q2 (reverse bs) cs))))
      (case (Cons z x2)
        (match y
          (case Nil
  ...
  +++2
    ...
    (match x
      (case (Q22 xs ys)
        (match y
          (case (Q22 vs ws)
            (mkQ2 (append2 xs (reverse2 ys)) (append2 ws (reverse2 vs))))))))
(define-fun-rec
  queue2
    ((x E2)) Q3
    (match x
  ...
  +++
    ...
    (match x
      (case (Q2 xs ys)
        (match y
          (case (Q2 vs ws)
            (mkQ (append xs (reverse ys)) (append ws (reverse vs))))))))
(define-fun-rec
  queue
    ((x E)) Q
    (match x
